...
An example file (`all_TFs_table_proteins_with_IDR.txt`) covering 1,590 human TFs is included in the repository.
...
An example file (`SFARI_TF_de_novo_variants_gpf.bed`) with 117 de novo variants from the SFARI database is included.
...
```bash
map-domains \
  --input            all_TFs_table_proteins_with_IDR.txt \
...
intersect-variants \
  --cds-dir      cds_beds/ \
  --variants     SFARI_TF_de_novo_variants_gpf.bed \
  --output-dir   output/
```
...
```bash
classify-domain-snvs \
  --domain-dir         domain_beds/ \
  --classified-snv-dir output/classified/ \
  --output-dir         output/domain_snvs/ \
  --mapping            all_TFs_table_proteins_with_IDR.txt
```
